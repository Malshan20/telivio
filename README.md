...
Every company that signs up becomes an **organization**. All data — jobs, candidates, interviews, notes, Calendly connections — is scoped to `organization_id`, and Postgres Row Level Security enforces that a user can only ever read or write rows belonging to their own organization. Company A's HR can never see Company B's candidates, and vice versa, even if they guess IDs or call the API directly.
...
organizations
  ...
  └── cal_connections (now stores Calendly tokens)  (org_id on every row, UNIQUE — one calendar per org)
```
...
## 📅 Calendly — OAuth, not a personal API key

This is the part that matters most for a multi-tenant product. Telivio does **not** use one shared Calendly API key. Instead:

1. Telivio itself has **one OAuth app** registered with Calendly (`CALENDLY_CLIENT_ID` / `CALENDLY_CLIENT_SECRET` below)
2. Each customer organization clicks **"Connect Calendly"** in Settings → Integrations
3. They're redirected to Calendly's consent screen, log into **their own** Calendly account, and approve
4. Telivio stores **their** access + refresh tokens in `cal_connections (now stores Calendly tokens)`, scoped to their `organization_id`
...
```
src/lib/cal.ts
  getCalAuthorizeUrl()       → builds the Calendly consent URL
  ...
  createSchedulingLink()     → books using THAT org's own Calendly account
  listEventTypes()           → lets an org pick which event type to use
  disconnectCal()            → removes a connection
```

### Setting up your Calendly OAuth app (one-time, by you — the platform owner)

1. Go to [calendly.com/integrate](https://calendly.com/integrate)
2. Choose **"Create an OAuth app and allow user to schedule"**
3. App name: `Telivio`
4. Redirect URI: `http://localhost:3000/api/calendly/callback` (add your production URL too)
5. Select booking + event-type scopes
6. Copy the **Client ID** and **Client Secret** into `.env.local`:
   ```env
   CALENDLY_CLIENT_ID=...
   CALENDLY_CLIENT_SECRET=...
   CALENDLY_REDIRECT_URI=http://localhost:3000/api/calendly/callback
   ```
...
| Interview Scheduling | Calendly OAuth — **per-organization** |
| Drag & Drop | @dnd-kit |

**Platform-level vs. per-organization:** Groq and Resend are configured once, by you, and used to serve every customer (they're how *you* deliver the AI scoring and email-sending service). Calendly is different — it's the one integration that's inherently tied to *whose calendar* the booking lands on, so it must be per-organization via OAuth.
...
- `organizations`, `users`, `jobs`, `candidates`, `interviews`, `notes`, `email_logs`, `cal_connections (now stores Calendly tokens)`, `usage_counters`, `organization_memberships`
...
4. Go to **Settings → Integrations → Connect Calendly** to link your own calendar
...
│   ├── cal.ts                # OAuth-based, per-organization Calendly integration
...
│   │   └── cal/
│   │       ├── connect/      # Starts OAuth flow (redirects to Calendly)
...
runRecruitingAgent(candidateId)
  ...
  │    → looks up THIS org's Calendly OAuth token         │
...
- Calendly tokens are stored server-side only, scoped one-to-one with `organization_id` (`UNIQUE` constraint), and refreshed automatically before expiry
...
Add all environment variables in the Vercel dashboard, and update `CALENDLY_REDIRECT_URI` (and the redirect URI registered in your Calendly OAuth app) to your production domain.
