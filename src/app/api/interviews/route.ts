...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create a scheduling link on THIS organization's own Cal.com account
    ...
    if (!link) {
      return NextResponse.json(
        { error: calError || 'Cal.com is not connected for this organization. Go to Settings → Integrations.' },
        { status: 400 }
      )
    }
