...
import { User, Save, Key, Bell, Zap, Copy, ExternalLink, Calendar, CheckCircle2, Loader2, Unlink, Building2, Mail, Palette } from 'lucide-react'
...
function SettingsContent() {
  ...
  useEffect(() => {
    ...
    if (connected) {
      toast.success('Cal.com connected! Interviews will now book onto your calendar.')
      setActiveTab('integrations')
      router.replace('/dashboard/settings')
    } else if (calError) {
      const messages: Record<string, string> = {
        no_organization: 'Set up your workspace before connecting Cal.com.',
        insufficient_permissions: 'Only workspace owners or admins can connect Cal.com.',
        missing_code: 'Cal.com did not return an authorization code. Try again.',
        ...
        exchange_failed: 'Failed to complete the Cal.com connection. Try again.',
        access_denied: 'You declined the Cal.com connection request.',
      }
      toast.error(messages[calError] || 'Failed to connect Cal.com')
      setActiveTab('integrations')
      router.replace('/dashboard/settings')
    }
  }, [searchParams, router])
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-surface-400 mt-1">
                  Groq and Resend are configured once for all of Telivio. <strong className="text-surface-300">Cal.com is different</strong> — every company connects its <em>own</em> Cal.com account above, so interviews always land on your team's calendar, never anyone else's.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between py-3 border-t border-surface-800">
                <div>
                  ...
                  <p className="text-xs text-surface-500">Send Cal.com links to qualified candidates</p>
                </div>
                <div className="w-11 h-6 bg-brand-600 rounded-full relative cursor-pointer">
                  <div className="w-4 h-4 bg-white rounded-full absolute right-1 top-1 shadow" />
                </div>
...
/**
 * Per-organization Cal.com connection card. Unlike Groq/Resend (configured
 * once for all of Telivio via env vars), this is genuinely per-tenant:
 * each company connects their OWN Cal.com account via OAuth, so interview
 * bookings land on their calendar — never Telivio's, never another
 * customer's.
 */
function CalConnectionCard() {
  const [status, setStatus] = useState<{
    connected: boolean
    calUsername?: string
    defaultEventTypeId?: string
  } | null>(null)
  const [eventTypes, setEventTypes] = useState<{ id: string; title: string; slug: string; length: number }[]>([])
  ...
  async function loadStatus() {
    try {
      const res = await fetch('/api/cal/status')
      const data = await res.json()
      setStatus(data)
      if (data.connected) {
        loadEventTypes()
  ...
  async function handleDisconnect() {
    if (!confirm('Disconnect Cal.com? New interview invites will fail to generate scheduling links until you reconnect.')) return
    setDisconnecting(true)
    try {
      const res = await fetch('/api/cal/disconnect', { method: 'POST' })
      if (!res.ok) throw new Error()
  ...
  if (!status) {
    return (
      <div className="card p-5 flex items-center gap-3">
        ...
        <span className="text-sm text-surface-500">Checking Cal.com connection…</span>
      </div>
    )
  }
    ...
    <div className="card p-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-surface-400 mt-0.5">
              {status.connected
                ? `Interview bookings go to your own Cal.com account${status.calUsername ? ` (@${status.calUsername})` : ''}.`
                : 'Connect your own Cal.com account so interviews booked through Telivio land on your team\'s calendar — not Telivio\'s.'}
            </p>
          </div>
        </div>
      ...
      {status.connected && (
        <div className="mt-4 pt-4 border-t border-surface-800">
          <label className="label">Default interview event type</label>
          <p className="text-xs text-surface-500 mb-2">
            Which Cal.com event type should Telivio use when scheduling interviews?
          </p>
          ...
          {loadingEventTypes ? (
            ...
            <p className="text-xs text-surface-600">
              No event types found. Create one in your Cal.com account first, then refresh this page.
            </p>
          ) : (
            <select
              className="input"
