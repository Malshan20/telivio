...
function SortableCard({ candidate }: { candidate: Candidate }) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <CandidateCard candidate={candidate} dragAttributes={Object.fromEntries(Object.entries(attributes || {})) as Record<string, unknown>} dragListeners={Object.fromEntries(Object.entries(listeners || {})) as Record<string, unknown>} />
    </div>
  )
}
          ...
          <div className="flex items-center gap-2 mt-2">
            {candidate.score !== null ? (
              <div className="flex items-center gap-1">
                ...
                <span className={`text-xs font-bold ${getScoreColor(candidate.score ?? 0)}`}>
                  {candidate.score}
                </span>
              </div>
            ) : (
