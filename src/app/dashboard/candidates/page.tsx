                      ...
                      <td className="px-5 py-4">
                        {c.score !== null ? (
                          <div className="flex items-center gap-2">
                            <span className={`text-sm font-bold ${getScoreColor(c.score ?? 0)}`}>{c.score}</span>
                            <div className="w-16 h-1.5 bg-surface-800 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${c.score ?? 0 >= 75 ? 'bg-emerald-500' : (c.score ?? 0) >= 50 ? 'bg-amber-500' : 'bg-red-500'}`}
