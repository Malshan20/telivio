...
// Lazy singleton — same reasoning as lib/groq.ts: never construct a
// third-party SDK client at module top-level, since Next.js imports every
// API route module during `next build`'s page-data-collection pass, and
// any constructor that throws (or a future SDK version that starts
// validating the key eagerly) would break the entire build.
