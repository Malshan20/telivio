...
import { createSchedulingLink } from '@/lib/cal'
 ...
 *
 * Every external action (Cal.com, candidate/job lookups, email logging) is
 * scoped to the candidate's organization_id, so this never crosses tenant
 * boundaries — Company A's agent run can never touch Company B's Cal.com
...
export async function runRecruitingAgent(candidateId: string): Promise<AgentResult> {
  ...
  if (decision === 'interview') {
    // INTERVIEW FLOW — uses THIS organization's own Cal.com connection
    try {
      ...
      // and gets a clear signal to connect/configure Cal.com).
      await supabase
        .from('interviews')
        .insert({
      ...
      // even if scheduling needs manual follow-up because Cal.com isn't connected.
      await supabase
        .from('candidates')
